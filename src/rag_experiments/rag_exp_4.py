...
hf = HuggingFacePipeline.from_model_id(
    ...
    device=-1,
    pipeline_kwargs={"max_new_tokens": 20, "top_p": 0.8}
)
...
# -----------------------------
# Save results along with experiment metadata
# -----------------------------
experiment_metadata = {
    "experiment": "Exp-3",
    ...
    "vectorstore": "FAISS (k=5)",
    "llm_model": llm_model_id,
    "prompt": "Strict copy-from-context, shortest exact phrase, or 'I don't know'",
    "generation_settings": {
        "max_new_tokens": 20,
        "top_p": 0.8
    }
...
output_path = Path(r"experiments_results/rag_answers_exp3.json")
output_path.parent.mkdir(parents=True, exist_ok=True)

with output_path.open("w") as f:
    json.dump(output_data, f, indent=2)

print(f"\nSaved answers and experiment metadata for {len(answers_dict)} questions to {output_path}")
